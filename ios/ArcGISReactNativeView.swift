...
public final class ArcGISReactNativeView: UIView {
    @objc public var eventHandler: ((String, [String: Any]) -> Void)?

    private let model = ReactNativeArcGISMapModel()
    private lazy var hostingController = UIHostingController(
        rootView: ReactNativeArcGISMapRoot(model: model, extensionHost: model.extensionHost)
    )

    private var markersById: [String: MarkerState] = [:]
    private let markerIngestQueue = DispatchQueue(
        label: "com.mapconductor.react.arcgis.marker-ingest",
        qos: .userInitiated
    )
    // These three properties are confined to markerIngestQueue. React Native invokes the
    // exported methods on the main thread, but decoding thousands of NSDictionary/NSArray
    // values and constructing Core MarkerState objects must not block map interaction.
    ...
    // Main-thread composition state. Updates issued immediately after commit are retained until
    // the background decoder's final result has been installed.
    ...
    @objc public func setMapDesignType(_ value: String?) {
        model.state.mapDesignType = arcGISDesign(forId: value)
    }
...
struct ReactNativeArcGISMapRoot: View {
    @ObservedObject var model: ReactNativeArcGISMapModel
    @ObservedObject var extensionHost: NativeMapExtensionHost

    var body: some View {
        if !model.isConfigured {
            Text("ArcGIS API key is not configured. Pass an `apiKey` prop to <ArcGISMapView />.")
            ...
            ArcGISMapView2D(
                ...
                sdkInitialize: { _ = arcGISApiKeyInitialize(apiKey: model.apiKey) },
                content: {
