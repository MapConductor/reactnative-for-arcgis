...
@interface MCArcGISReactNativeView : UIView
...
@interface MapConductorArcGISContainerView : UIView
@property(nonatomic, strong) MCArcGISReactNativeView *mapView;
...
@implementation MapConductorArcGISContainerView

- (instancetype)init
{
  if ((self = [super init])) {
    Class mapViewClass = NSClassFromString(@"MCArcGISReactNativeView");
    NSAssert(mapViewClass != Nil, @"MapConductorForArcGIS XCFramework is not linked");
...
// duplicate-method compile error). `ArcGISMapView` isn't a Codegen'd Fabric component (it predates
// codegen adoption in this repo, matching the Google Maps and MapLibre RN bridges), so it needs
// this explicit legacy-interop registration to avoid "View config not found for component
// `ArcGISMapView`" under bridgeless New Architecture.
...
static void MCArcGISRegisterLegacyViewManagerInterop(void)
{
...
#pragma clang diagnostic ignored "-Warc-performSelector-leaks"
    [cls performSelector:selector withObject:@"ArcGISMapView"];
...
- (UIView *)view
{
  return [MapConductorArcGISContainerView new];
}
...
RCT_CUSTOM_VIEW_PROPERTY(cameraPosition, NSDictionary, MapConductorArcGISContainerView)
{
  if (json) [view.mapView setCameraPosition:json];
}

RCT_CUSTOM_VIEW_PROPERTY(apiKey, NSString, MapConductorArcGISContainerView)
{
  [view.mapView setApiKey:json];
}

RCT_CUSTOM_VIEW_PROPERTY(mapDesignType, NSString, MapConductorArcGISContainerView)
{
  [view.mapView setMapDesignType:json];
}

RCT_CUSTOM_VIEW_PROPERTY(markerTilingOptions, NSDictionary, MapConductorArcGISContainerView)
{
  [view.mapView setMarkerTilingOptions:json viewId:view.reactTag.integerValue];
}

RCT_CUSTOM_VIEW_PROPERTY(infoBubblePositions, NSArray, MapConductorArcGISContainerView)
{
  [view.mapView setInfoBubblePositions:json ?: @[]];
}

- (void)withView:(NSNumber *)reactTag block:(void (^)(MCArcGISReactNativeView *))block
{
  [self.bridge.uiManager addUIBlock:^(__unused RCTUIManager *manager, NSDictionary<NSNumber *, UIView *> *registry) {
    ...
    if (![view isKindOfClass:[MapConductorArcGISContainerView class]]) return;
    block(((MapConductorArcGISContainerView *)view).mapView);
  }];
}

RCT_EXPORT_METHOD(clearOverlays:(nonnull NSNumber *)reactTag)
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) { [view clearOverlays]; }];
}

RCT_EXPORT_METHOD(moveCamera:(nonnull NSNumber *)reactTag position:(nonnull NSDictionary *)position)
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) { [view moveCamera:position duration:0]; }];
}
...
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) { [view moveCamera:position duration:duration]; }];
}

RCT_EXPORT_METHOD(fitBounds:(nonnull NSNumber *)reactTag bounds:(nonnull NSDictionary *)bounds padding:(NSInteger)padding)
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) { [view fitBounds:bounds padding:padding]; }];
}
...
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) { [view beginMarkerComposition:generation icons:icons]; }];
}
...
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) {
...
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) { [view commitMarkerComposition:generation]; }];
}

RCT_EXPORT_METHOD(updateMarker:(nonnull NSNumber *)reactTag payload:(nonnull NSDictionary *)payload)
{
  [self withView:reactTag block:^(MCArcGISReactNativeView *view) { [view updateMarker:payload]; }];
}
