...
import com.mapconductor.react.arcgis.marker.ReactNativeMarkerIcon
import com.mapconductor.react.arcgis.marker.fromReadableMap
import com.mapconductor.react.arcgis.marker.toMarkerIcon
...
class ArcGISMapViewWrapper(context: Context) :
    ...
    companion object {
        ...
        private val markerIngestDispatcher: CoroutineDispatcher =
            Executors.newSingleThreadExecutor { r ->
                Thread(r, "ArcGISMarkerIngest").apply { isDaemon = true }
    ...
    private val extensionScope = ArcGISMapViewScope()
    ...
    fun initializeMapIfNeeded() {
        ...
        ensureArcGISInitialized(context, pendingApiKey)
        val nativeMapView = com.arcgismaps.mapping.view.MapView(context)
        val wrapView =
            WrapMapView(context).apply {
                addView(nativeMapView, LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT)
            }
        wrapView.arcGISMapView = nativeMapView
        wrapMapView = wrapView
        addView(wrapView, 0, LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT))
        ...
        mainCoroutine.launch {
            val controller =
                createArcGISMapViewController(
                    wrapView = wrapView,
                    mapDesignType = pendingMapDesign,
            ...
            wrapView.post {
                controller.moveCamera(initialCamera)
    ...
    fun setMapDesignType(mapDesignType: String?) {
        val design = ArcGISReactNativeDesign.from(mapDesignType)
    ...
    override fun onLayout(
        ...
        wrapMapView?.layout(0, 0, right - left, bottom - top)
    ...
    private fun markerTrace(message: String) {
        Log.d(
            MARKER_TRACE_TAG,
            "[ArcGIS][RN][t=${SystemClock.elapsedRealtime()}]" +
     ...
     * overlay controllers on the main thread (the view is gone, but a stale in-flight update
     * isn't an error worth crashing the app over). Swallow only that specific race; anything
     * else, including cancellation, propagates normally.
     */
